...
//  计算全选状态
const isSelectdALL = computed(() => {
  return cartList.value?.length && cartList.value?.every((v) => v.selected)
})
// 全选
const onChangeSelectedAll = () => {
  // 将当前全选状态取反
  const _isSelectdALL = !isSelectdALL.value
  cartList.value?.forEach((item) => {
    return (item.selected = _isSelectdALL)
  })
  putMemberCartSelectedAPI({ selected: _isSelectdALL })
}
// 计算选中商品列表
...
let style = ref({})
if (pages.length > 0) {
  const pageArr = pages[pages.length - 1]
  if (pageArr.route != 'pages/cart/cart') {
...
<template>
  <scroll-view scroll-y class="scroll-view" @scrolltolower="onScrolltolower">
    ...
    <template v-if="memberStore.profile">
      ...
      <!-- 吸底工具栏 -->
      <view class="toolbar" :style="style">
        <text class="all" :class="{ checked: isSelectdALL }" @tap="onChangeSelectedAll">全选</text>
