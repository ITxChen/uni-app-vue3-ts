...
const getMemberOrderPreData = async () => {
  if (query?.count && query?.skuId) {
    ...
    // 再次够买
    const res = getMemberOrderRepurchaseByIdAPI(query.orderId)
    orderPre.value = (await res).result
  } else {
    // 预支付
...
<template>
  <scroll-view scroll-y class="viewport">
    ...
    <!-- 支付金额 -->
    <view class="settlement" style="padding-bottom: 120px">
      <view class="item">
  ...
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    ...
    <view class="button" :class="{ disabled: selectAddress?.id }" @tap="onOderSubmit">
      提交订单
    </view>
  </view>
</template>
